...
class HotelReservation(models.Model):
    ...
    def _check_reservation_rooms(self):
        ...
        for reservation in self:
            ...
            if len(reservation.reservation_line) > 0 and cap > 0: #added by priyanka
    ...
    def create(self, vals):
        ...
        """
        print(self.env["ir.sequence"].next_by_code("hotel.reservation"))
        vals["reservation_no"] = (
    ...
    def confirmed_reservation(self):
        ...
        for reservation in self:
            ...
            for line_id in reservation.reservation_line:
                ...
                if(len(line_id.checkin_lines)==0):
                    raise ValidationError(
                        _(
                            "Please provide the checkin details."
                        ))

                ###added by priyanka###
                print(len(line_id.reserve))
                if line_id.categ_id and len(line_id.checkin_lines) > 0:
                    roomObj = self.env["hotel.room"]
                    #for room in roomObj.search([("room_categ_id", "=", line_id.categ_id.id)]):
                        #for reserv in room.room_reservation_line_ids.search([("status", "in", ("confirm", "done"))]):
                    for reserv in line_id.checkin_lines:
                            check_in = reserv.check_in
                            check_out = reserv.check_out
                            if check_in <= reserv_checkin <= check_out:
                                room_bool = True
...
class HotelRoomReservationLine(models.Model):
    ...
    state = fields.Selection(
        [("par assigned", "Partially Assigned"),("assigned", "Assigned"), ("unassigned", "Unassigned")], "Room Status"
    )
    ...
    occupancy = fields.Integer(string="Occupancy") #priyanka
